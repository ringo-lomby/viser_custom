...
function useMessageHandler() {
  ...
  const addUnifiedHudHtml = viewer.useGui((state: GuiState & GuiActions) => state.addUnifiedHudHtml);
  ...
  return (message: Message) => {
    if (isGuiComponentMessage(message)) {
      if (message.type === "GuiHtmlMessage") {
        ...
        } else if (["speed-gauge", "battery", "mission-info", "traffic-light", "notification", "stop_reason"].includes(message.props.target_area)) {
